...
--------------------------------------------------------------------------------------
--------------------------------------------------------------------------------------
# CONSTRUCTING FOURTH CHART AND SAVING AS PNG (plot4.png)
png(file = "./Data/exdata_data_household_power_consumption/plot4.png")
par(mfrow = c(2,2)) # implies one row and two columns
par(mar = c(2,2,2,2))

with(power_trunc, plot(datetime, Global_active_power,type="l", ylab = "Global Active Power", xlab = ""))
with(power_trunc, plot(datetime, Voltage, type="l", xlab = "datetime"))
with(power_trunc, plot(datetime, Sub_metering_1,type="n", yayp  = c(0, 35, 10), ylab = "Energy sub metering", xlab = ""))
...
with(power_trunc, plot(datetime, Global_reactive_power, type="l", xlab = "datetime"))
dev.off()
